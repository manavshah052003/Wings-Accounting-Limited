  ...
  return (
    <>
      ...
      <section
        id="specialisations"
        ref={specRef}
        className="py-40 lg:py-56 bg-white flex flex-col items-center"
      >
        <div className="max-w-6xl mx-auto px-6">
          ...
          >
            <span className="section-tagline mb-3">{specialisationsContent.tagline}</span>
            <h2 className="text-3xl lg:text-4xl font-black text-[#1a3a5c] leading-tight text-center">
            ...
            {specialisationsContent.items.map((item, i) => {
              ...
              return (
                <motion.div
                  ...
                  initial="hidden"
                  animate={specInView ? "visible" : "hidden"}
                  className="group relative rounded-2xl overflow-hidden bg-[#f8fafc] hover:bg-[#1a3a5c] border border-[#e2e8f0] hover:border-transparent transition-all duration-400 hover:shadow-2xl hover:shadow-[#1a3a5c]/20 hover:-translate-y-2"
                >
                  <div className="p-8">
                    <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#29b79b]/15 to-[#1a3a5c]/10 group-hover:from-[#29b79b] group-hover:to-[#1d9e83] flex items-center justify-center mb-6 transition-all duration-300 shadow-sm group-hover:shadow-lg group-hover:shadow-[#29b79b]/30">
                      <Icon
                        size={26}
                        className="text-[#29b79b] group-hover:text-white transition-colors duration-300"
                      />
                    </div>
                    <h3 className="font-black text-[#1a3a5c] group-hover:text-white text-xl mb-3 transition-colors duration-300">
                      {item.title}
                    </h3>
                    <p className="text-[#64748b] group-hover:text-white/70 text-sm leading-relaxed transition-colors duration-300">
                      {item.description}
                    </p>
                  </div>

                  {/* Arrow badge */}
                  <div className="absolute top-5 right-5 w-8 h-8 rounded-full bg-white/0 group-hover:bg-[#29b79b]/20 flex items-center justify-center transition-all duration-300">
                    <ArrowUpRight
                      size={14}
